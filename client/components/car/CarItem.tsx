...
const CardItem = ({ car }: { car: ICar }) => {
  return (
    <Card className='m-2 my-4 py-0'>
      <div className='flex items-center bg-white shadow-md rounded-lg'>
        <div className='w-48 h-48'>
          {/* TODO: Fix car image urls */}
          {car?.images[0]?.url ? (
            <Image src={car?.images[0]?.url} alt={car?.name} className='h-full w-full object-cover rounded-l-lg' width={192} height={192} />
          ) : (
